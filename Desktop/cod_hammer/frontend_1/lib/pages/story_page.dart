...
class StoryPage extends StatefulWidget {
  ...
  const StoryPage({Key? key, required this.orderHistory}) : super(key: key);
  ...
  void _loadDummyOrderHistory() {
    ...
    _orderHistory = [
      Order(
        ...
        totalAmount: orderItems1.fold(0, (sum, item) => sum + item.product.price * item.quantity),
      ),
      Order(
        orderId: '67890',
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: _orderHistory.isEmpty
          ? const Center(
              child: Text('История заказов пуста.', style: TextStyle(color: textColorSecondary)),
            )
          : ListView.builder(
                    ...
                    child: Row(
                      children: [
                        SizedBox(
                          width: 70,
                          height: 70,
                          child: order.orderItems.isNotEmpty ? AvatarWidget(
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              Text('Заказ №: ${order.orderId}', style: const TextStyle(color: secondaryColor, fontWeight: FontWeight.bold)),
                              Text('Дата: ${formatDate(order.orderDate)}', style: const TextStyle(color: textColorSecondary)),
                            ],
                          ),
                        ),
                        Text(formatNumber(order.totalAmount), style: const TextStyle(color: tertiaryColor, fontWeight: FontWeight.bold, fontSize: 16)),
                      ],
                    ),
                  ),
                );
